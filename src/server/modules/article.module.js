  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        connection.query('INSERT INTO lunch_detail SET ?', insertValues, (error, result) => { // Article資料表寫入一筆資料
          if (error) {
            console.error('SQL error: ', error);
            reject(error); // 寫入資料庫有問題時回傳錯誤
  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        connection.query('UPDATE lunch_detail SET ? WHERE AutoInc = ?', [insertValues, userId], (error, result) => {
          if (error) {
  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        connection.query('DELETE FROM lunch_detail WHERE AutoInc = ?', userId, (error, result) => {
          if (error) {
